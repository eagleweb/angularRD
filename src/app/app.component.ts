...
import {Observable, of, from, fromEvent, throwError, Subscription, merge} from 'rxjs';
import { concat, map, reduce, concatMap, first, last, delay, filter, catchError, mergeMap, tap, window } from 'rxjs/operators';
import {bufferCount, partition, pluck, throttleTime} from "rxjs/internal/operators";
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    const notifications = [
      ...
      // { userId: 3, name: 'C1', delay: 400 }, // should be shown
      // { userId: 3, name: 'C2', delay: 2000 }, // should be shown
      // { userId: 3, name: 'C3', delay: 5000 }, // should be shown
    ];
    ...
    const [user1, user2] = messenger.pipe(partition(val => val.userId === 1));
    const sub = merge(
      user1.pipe(
        pluck('name'),
        throttleTime(3000)
    ...
    this.subscriptions.add(sub.subscribe(val => console.log(val)));
  }
